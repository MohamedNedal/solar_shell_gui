...
import requests # For downloading GONG data
from bs4 import BeautifulSoup # For parsing HTML to find GONG links
import urllib.request # For downloading files
import gzip # For unzipping GONG files
...
# from astropy import constants as const # For solar radius and other constants
...
class AIAViewer(QWidget):
    def __init__(self):
        ...
        # Use a temporary directory for GONG data to avoid permission issues
        self.data_dir = os.path.join(tempfile.gettempdir(), 'solar_shell_data')
        ...
        self.init_ui()
        # Load magnetic field data once at startup
        # This uses a hardcoded date for now, consider linking to map.date later
        self.load_magnetic_field_data('2011-06-07T06:15') 
    ...
    def find_links(self, url):
        ...
        try:
            ...
            for a in soup.find_all('a'):
                href = a.get('href')
                if href and href.endswith('.gz'): # Only interested in .gz files for GONG
                    hrefs.append(href)
            return hrefs
        except requests.exceptions.RequestException as e:
            QMessageBox.critical(self, 'Network Error', f'Could not access GONG URL: {url}\nError: {e}')
            return []

    def load_magnetic_field_data(self, mydatetime_str):
        """
        Extrapolate the coronal magnetic field using the PFSS model
        and trace field lines.
        """
        self.label.setText('Loading magnetic field data...')
        QApplication.processEvents() # Update GUI immediately

        try:
            YEAR, MONTH, DAY = mydatetime_str.split('T')[0].split('-')
            HOUR, MINUTE     = mydatetime_str.split('T')[1].split(':')

            # Construct GONG URL and find file
            gong_url = 'https://gong2.nso.edu/oQR/zqs/%s%s/mrzqs%s%s%s/' %(YEAR, MONTH, YEAR[-2:], MONTH, DAY)
            list_gong_links = self.find_links(url=gong_url)

            if not list_gong_links:
                self.label.setText('No GONG files found for the specified date.')
                return

            # Try to find a suitable GONG file, e.g., the first one or a specific pattern
            gong_filename = None
            for link in list_gong_links:
                if 'mrzqs' in link: # Look for magnetogram files
                    gong_filename = link
                    break
            
            if gong_filename is None:
                self.label.setText('No suitable GONG magnetogram file found.')
                return

            full_gong_path_gz = os.path.join(self.data_dir, gong_filename)
            full_gong_path_unzipped = full_gong_path_gz[:-3] # Remove .gz

            # Download and unzip if not exists
            if not os.path.exists(full_gong_path_unzipped):
                if not os.path.exists(full_gong_path_gz):
                    self.label.setText(f'Downloading {gong_filename}...')
                    QApplication.processEvents()
                    urllib.request.urlretrieve(url=gong_url+gong_filename, filename=full_gong_path_gz)
                
                self.label.setText(f'Unzipping {gong_filename}...')
                QApplication.processEvents()
                with gzip.open(full_gong_path_gz, 'rb') as f_in:
                    with open(full_gong_path_unzipped, 'wb') as f_out:
                        f_out.write(f_in.read())
                os.remove(full_gong_path_gz) # Clean up .gz file

            # Make a sunpy map of the file
            gong_map = sunpy.map.Map(full_gong_path_unzipped)
            ...
            rss      = 2.5 # source surface radius (common value, adjust if needed)
            ...
            self.label.setText('Magnetic field data loaded.')

        except Exception as e:
            self.label.setText(f'Error loading magnetic field data: {e}')
            self.pfss_out = None
    ...
    def create_3d_ellipsoid(self, ellipse_params, sunpy_map, show_shell=True, show_normals=False, show_radials=False, n_lat_radial=10, n_lon_radial=30, show_field_lines=False):
        ...
        if show_radials:
            radial_length = 2.0 # Length of radial lines in solar radii
        ...
        if len(all_points_for_theta) > 0 and len(x_outer) > 0:
            for i in range(len(x_outer)):
                ...
                cos_theta = np.clip(cos_theta, -1.0, 1.0) # Ensure within valid range for arccos
    ...
    def show_3d_ellipsoid_plot(self):
        ...
        if self.field_lines is None and self.show_field_lines_checkbox.isChecked():
            QMessageBox.warning(self, 'Warning', 'Magnetic field data is not loaded. Please ensure internet connection and try again, or uncheck "Show Magnetic Field Lines".')
            return

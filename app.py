...
class AIAViewer(QWidget):
    ...
    def draw_ellipse(self, redraw=True):
        ...
        try:
            ...
            if not self.maps or not self.figure.axes:
                self.label.setText('No map displayed to draw ellipse on.')
                return
            ...
            # Create SkyCoord with the same frame as the map
            coords = SkyCoord(
                x_ell * u.arcsec, 
                y_ell * u.arcsec, 
            ...
            # Draw new ellipse - use transform_to to convert to the map's coordinate system
            self.ellipse_artist, = ax.plot_coord(
                coords.transform_to(amap.coordinate_frame), 
                color='red', 
                lw=2
            )
    ...
    def update_ellipse(self):
        ...
        try:
            ...
            transformed_coords = coords.transform_to(amap.coordinate_frame)
            
            # If artist exists, update it
            if self.ellipse_artist is not None:
                # Update existing artist
                self.ellipse_artist.set_data(
                    transformed_coords.Tx.value, 
                    transformed_coords.Ty.value
                )
            else:
                # Create new artist
                ax = self.figure.axes[0]

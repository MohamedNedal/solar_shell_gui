...
class AIAViewer(QWidget):
    ...
    def load_files(self):
        """
        Open a file dialog to select FITS files, load them into SunPy Maps
        sorted chronologically, and display the first frame.
        """
        ...
        self.maps, self.files = load_fits_files(filepaths, progress_callback=_cb)
        self.current_index = 0
        self.progress.setVisible(False)
    ...
    def run_running_diff(self):
        """
        Compute a running-difference movie from the current processed maps.

        Requires at least 6 frames.  Each difference map is Gaussian-smoothed
        (sigma=3) and uses a symmetric ±50 DN/s colour scale.  The result
        replaces the active map sequence.
        """
        self.set_processed_maps_from_loaded()
        if len(self.processed_maps) < 6:
            QMessageBox.warning(
                self, 'Warning', 'Need at least 6 images for running difference.')
            return
        ...
        try:
            self.running_diff_maps = create_running_diff_maps(
                self.processed_maps, progress_callback=_cb)
